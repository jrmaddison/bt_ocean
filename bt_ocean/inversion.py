...
class ModifiedHelmholtzSolver:
    ...
    def solve(self, b):
        ...
        a_k = -4 * jnp.sin(jnp.arange(1, self.grid.N_x, dtype=self.grid.fdtype) * jnp.pi / (2 * self.grid.N_x)) ** 2 / (self.grid.dx ** 2)
        a_l = -4 * jnp.sin(jnp.arange(1, self.grid.N_y, dtype=self.grid.fdtype) * jnp.pi / (2 * self.grid.N_y)) ** 2 / (self.grid.dy ** 2)
