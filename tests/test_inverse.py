...
def test_poisson_solver_residual(L_x, L_y, N_x, N_y):
    ...
    b = (solver.grid.D_xx @ u + u @ solver.grid.D_yy.T)[1:-1, 1:-1]
    error_norm = abs(b - b_ref[1:-1, 1:-1]).max() / abs(b).max()
    print(f"{error_norm=}")
    assert error_norm < (1.0e3, 1.0e4)[int(jax.config.x64_enabled)] * eps()


@pytest.mark.parametrize("L_x, L_y", [(1, 1),
                                      (cbrt(3), cbrt(2))])
@pytest.mark.parametrize("N_x, N_y", [(50, 50),
                                      (50, 100),
                                      (100, 50)])
def test_poisson_solver_convergence(L_x, L_y, N_x, N_y):
    solver = PoissonSolver(Grid(L_x, L_y, N_x, N_y))

    X, Y = solver.grid.X, solver.grid.Y
    b = (jnp.sin((jnp.pi * (X + L_x)) / (2 * L_x))
         * jnp.sin((3 * jnp.pi * (Y + L_y)) / (2 * L_y)))
    u = solver.solve(b)
...
def test_modified_helmholtz_solver_residual(L_x, L_y, N_x, N_y, alpha):
    ...
    b = (solver.grid.D_xx @ u + u @ solver.grid.D_yy.T - alpha * u)[1:-1, 1:-1]
    error_norm = abs(b - b_ref[1:-1, 1:-1]).max() / abs(b).max()
    print(f"{error_norm=}")
    assert error_norm < (1.0e3, 1.0e4)[int(jax.config.x64_enabled)] * eps()
...
@pytest.mark.parametrize("N_x, N_y", [(50, 50),
                                      ...
                                      (100, 50)])
@pytest.mark.parametrize("alpha", (0.5, 1, 2, exp(-0.5)))
def test_modified_helmholtz_solver_convergence(L_x, L_y, N_x, N_y, alpha):
    solver = ModifiedHelmholtzSolver(
        Grid(L_x, L_y, N_x, N_y), alpha=alpha)
    ...
    b = (jnp.sin((jnp.pi * (X + L_x)) / (2 * L_x))
         * jnp.sin((3 * jnp.pi * (Y + L_y)) / (2 * L_y)))
    u = solver.solve(b)
    u_ref = -b / ((jnp.pi ** 2) * (1 / (2 * L_x) ** 2 + 9 / ((2 * L_y) ** 2)) + alpha)
    error_norm = abs(u - u_ref).max()
    print(f"{error_norm=}")
